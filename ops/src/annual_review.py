...
def section_sensitivity() -> Section:
    ...
    rows = [
        ...
        "### 這兩個軸是怎麼來的(讀數字前先看這段)",
        "",
        "**沒有疑問的部分:**「現行」那一格確實就是你在跑的設定 —— "
        f"`MA={CURRENT_MA}` 是趨勢腿實際的均線長度,`mult={CURRENT_MULT}` 定義為"
        "**現行實作的門檻原封不動**(`0.5 / 1.0 / 1.8 / 2.6 / 3.4`),"
        "`mult` 的作用是把整組 VIX 階梯門檻等比例縮放。所以百分位與鄰域離散度"
        "算的是你真正在用的東西,這點沒有錯。",
        "",
        "⚠ **但這兩個軸是本專案自訂的,不是原始參數化方式的重現。** "
        "原始描述寫的是 `MA12 / lvl1.2`,而現行 `lvl = (VIX−16)/9` 裡沒有 `1.2` —— "
        "2026-08-12 確認**原始定義已不可考**。因此:",
        "",
        "- 本節掃的是「均線長度」與「VIX 門檻整體縮放」這兩個維度",
        "- 其他可能敏感的維度(`lvl` 的分母 9、風險分數的 `0.55/0.30/0.15` 權重)"
        "**沒有被掃到**",
        "- 後果是:數字不會錯,但**安全感可能不完整** —— 本節說「參數在高原上」時,"
        "指的只是這兩個方向上是高原,不保證沒被掃到的方向也平坦",
        "",
        "這一節是**一年一次的決策輔助,不是運作依賴** —— 它算錯或掃不全都不會影響 "
        "PWA 的任何輸出(現金水位由黃金基準釘死)。若日後找到原始定義,"
        "更新 `annual_review.py` 的 `MA_GRID` / `MULT_GRID` 即可。",
    ]
